...
def safe_accumulate_mean2d_gradient(variables):
    if 'means2D' not in variables:
        print("[Warning] 'means2D' not in variables. Skipping accumulation.")
        return variables
    if variables['means2D'].grad is None:
        print("[Warning] 'means2D.grad' is None. Skipping accumulation.")
        return variables

    grad = torch.norm(
        variables['means2D'].grad[variables['seen'], :2], dim=-1
    )
    variables['means2D_gradient_accum'][variables['seen']] += grad
    variables['denom'][variables['seen']] += 1.0
    return variables
...
def remove_points(to_remove, params, variables, optimizer):
    ...
    if 'seen' in variables:
        ...
        if variables['seen'].shape[0] == to_keep.shape[0]:
            variables['seen'] = variables['seen'][to_keep]
        else:
            print(f"[Warning] Skipping '{'seen'}' due to shape mismatch: "
                  f"mask={to_keep.shape[0]} vs {'seen'}={variables['seen'].shape[0]}")

    return params, variables


def safe_remove_points(to_remove, params, variables, optimizer):
    to_keep = ~to_remove
    keys = [k for k in params.keys() if k not in ['cam_unnorm_rots', 'cam_trans']]
    ...
    for key in ['means2D_gradient_accum', 'denom', 'max_2D_radius', 'timestep', 'seen']:
        if key in variables:
            if variables[key].shape[0] == to_keep.shape[0]:
                variables[key] = variables[key][to_keep]
            else:
                print(f"[Warning] Skipping '{key}' due to shape mismatch: "
                      f"mask={to_keep.shape[0]} vs {key}={variables[key].shape[0]}")

    if 'means2D' in variables:
        del variables['means2D']
...
def densify(params, variables, optimizer, iter, densify_dict):
    if iter <= densify_dict['stop_after']:
        ...
        if (iter >= densify_dict['start_after']) and (iter % densify_dict['densify_every'] == 0):
            ...
            new_params = {
                k: v[to_clone] if v.shape[0] == to_clone.shape[0] else v
            ...
            new_params = {
                k: v[to_split].repeat(n, 1) if v.shape[0] == to_split.shape[0] else v
                for k, v in params.items()
            }
            ...
            if params['log_scales'].shape[1] == 1:
                log_scales = torch.tile(params['log_scales'], (1, 3))  # isotropic
            else:
                log_scales = params['log_scales']  # anisotropic
            assert log_scales.shape[1] == 3, f"log_scales shape mismatch: {log_scales.shape}"
            stds = torch.exp(log_scales)[to_split].repeat(n, 1)

            means = torch.zeros((stds.size(0), 3), device="cuda")
            samples = torch.normal(mean=means, std=stds)
            rots = build_rotation(params['unnorm_rotations'][to_split]).repeat(n, 1, 1)  # 扰动
            # new_params['means3D'] += torch.bmm(rots, samples.unsqueeze(-1)).squeeze(-1)
            samples_transformed = torch.bmm(rots, samples.unsqueeze(-1)).squeeze(-1)
            print(f"new_params['means3D'].shape = {new_params['means3D'].shape}")
            print(f"samples_transformed.shape = {samples_transformed.shape}")

            new_params['means3D'] = new_params['means3D'] + samples_transformed
...
def fine_prune_gaussians(params, variables, optimizer, iter, prune_dict):
    if iter <= prune_dict['stop_after']:
        if (iter >= prune_dict['start_after']) and (iter % prune_dict['prune_every'] == 0):
            if iter == prune_dict['stop_after']:
                remove_threshold = prune_dict['final_removal_opacity_threshold']
            ...
            if iter >= prune_dict['remove_big_after']:
                big_points_ws = torch.exp(params['log_scales']).max(dim=1).values > 0.1 * variables['scene_radius']
                to_remove = torch.logical_or(to_remove, big_points_ws)
            params, variables = safe_remove_points(to_remove, params, variables, optimizer)
            torch.cuda.empty_cache()
...
def fine_densify(params, variables, optimizer, iter, densify_dict):
    if iter > densify_dict['stop_after']:
        return params, variables

    if 'means2D' not in variables:
        print("[Info] Skipping densify — 'means2D' not found in variables.")
        return params, variables
    if variables['means2D'].grad is None:
        print("[Info] Skipping densify — 'means2D.grad' is None.")
        return params, variables
    ...
    if (iter < densify_dict['start_after']) or (iter % densify_dict['densify_every'] != 0):
        return params, variables
    ...
    # optional: prune recently split points
    to_remove = torch.cat((to_split, torch.zeros(n * to_split.sum(), dtype=torch.bool, device="cuda")))
    params, variables = remove_points(to_remove, params, variables, optimizer)

    # prune low-opacity/big ones
    if iter == densify_dict['stop_after']:
        remove_threshold = densify_dict['final_removal_opacity_threshold']
    else:
        remove_threshold = densify_dict['removal_opacity_threshold']
