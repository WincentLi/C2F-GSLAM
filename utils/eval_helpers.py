...
'''
dataset: 根据不同的数据集格式，把image+depth放入一个列表，这里放的是gt
final_params:位姿、图像信息、深度图信息、时间戳、内参，最主要的就是这个了，现在要弄清楚这个东西的构成
num_frames:数量
eval_dir:输出路径
sil_thres:0.5,增加高斯的阈值
mapping_iters:建图30次迭代
...
1. initializaion:
    params = {
        ...
        'unnorm_rotations': unnorm_rots,  # (N,4)  # 渲染前进行normalize
        'logit_opacities': logit_opacities,  # (N,1)  # 渲染前进行sigmod
        'log_scales': log_scales,  # (N,1) or (N,3) # 渲染前进行exp
        'cam_unorm_rots': cam_rots  # (1,4,N)  # 渲染前需要进行normalize
        'cam_trans': cam_trans  # (1,3,N)  后续不需要任何处理
    }
'''
...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
    ...
    psnr_list = []  # psnr列表
    rmse_list = []  # rmse列表/tracking
    l1_list = []  # depth-l1列表
    lpips_list = []  # lpips列表
    ssim_list = []  # ssim列表
    plot_dir = os.path.join(eval_dir, "plots")  # 绘图到同一个image下
    os.makedirs(plot_dir, exist_ok=True)
    if save_frames:  # 构建几个文件夹，先渲染然后再统一计算结果
    ...
    for time_idx in tqdm(range(num_frames)):
         # Get RGB-D Data & Camera Parameters
        color, depth, intrinsics, pose = dataset[time_idx]  # 取rgb，depth，内参，pose
        if torch.isinf(pose).any() or torch.isnan(pose).any():
            print(f"Skipping invalid frame at index {time_idx} due to inf/nan pose.")
            continue

        gt_w2c = torch.linalg.inv(pose)  # 世界坐标系到相机坐标系的地面真值
        gt_w2c_list.append(gt_w2c)  # 为什么要转换呢
        intrinsics = intrinsics[:3, :3]  # 为什么取前三个呢

        # Process RGB-D Data
        color = color.permute(2, 0, 1) / 255 # (H, W, C) -> (C, H, W)  # 真值
        depth = depth.permute(2, 0, 1) # (H, W, C) -> (C, H, W)  # gt

        if time_idx == 0:
            # Process Camera Parameters
            first_frame_w2c = torch.linalg.inv(pose)
            # Setup Camera  和主函数一样设置cam
            cam = setup_camera(color.shape[2], color.shape[1], intrinsics.cpu().numpy(), first_frame_w2c.detach().cpu().numpy())
        ...
        # Get current frame Gaussians  将高斯转到frame坐标系下
        transformed_gaussians = transform_to_frame(final_params, time_idx, 
                                                   gaussians_grad=False, 
                                                   camera_grad=False)
 
        # Define current frame data  GT
        curr_data = {'cam': cam, 'im': color, 'depth': depth, 'id': time_idx, 'intrinsics': intrinsics, 'w2c': first_frame_w2c}

        # Initialize Render Variables  固定搭配了属于
        ...
        # Render RGB and Calculate PSNR
        im, radius, _, = Renderer(raster_settings=curr_data['cam'])(**rendervar)
        if mapping_iters==0 and not add_new_gaussians:  # 这是什么意思，不建图吗 or add_new_gaussians == False 不添加高斯，这是在干什么？？
            weighted_im = im * presence_sil_mask * valid_depth_mask  # 仅仅保留有意义的像素进行psnr计算
    ...
    # 写入 result.txt
    result_text = (
        "Final Average ATE RMSE: {:.3f} cm\n"
        "Average PSNR: {:.3f}\n"
        "Average Depth RMSE: {:.3f} cm\n"
        "Average Depth L1: {:.3f} cm\n"
        "Average MS-SSIM: {:.3f}\n"
        "Average LPIPS: {:.3f}\n"
    ).format(ate_rmse*100, avg_psnr, avg_rmse * 100, avg_l1 * 100, avg_ssim, avg_lpips)

    result_txt_path = os.path.join(eval_dir, 'result.txt')
    ...
    if wandb_run is not None:
        ...
        summary_items = sorted(summary_dict.items())  # 可选：按键排序输出

        summary_lines = []
        for k, v in summary_items:
            summary_lines.append(f"{k}: {v}")

        wandb_summary_path = os.path.join(eval_dir, "wandb_summary.txt")
        with open(wandb_summary_path, "w") as f:
            f.write("\n".join(summary_lines))

        print(f"[INFO] Saved wandb summary to {wandb_summary_path}")
